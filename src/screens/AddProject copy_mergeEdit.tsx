...
const AddProject = ({ edit }: AddProjectProps) => {
    ...
    if (edit && params.id) {
      const fetchProjectDetail = async () => {
        try {
          ...
          setStartDate(new Date(response.from));
          setEndDate(new Date(response.to));
          
          const [baseAddress, ...detailParts] = response.address.split(' ');
          setLocation({
          ...
          // if (response.users) {
          //   const userOptions = response.users.map((user: User) => ({
          //     value: user.id,
          //     label: user.name
          //   }));
          ...
          //   setRecommendedDates(response.preferences.map((date: string) => new Date(date)));
          // }
        } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!edit) {
      projectData.preferences = recommendedDates.map(date => date.toISOString().split('T')[0]) as string[];
    }

    try {
      if (edit && params.id) {
        await projectApi.updateProject(params.id as string, projectData);
      ...
      console.error(edit ? '프로젝트 수정 실패:' : '프로젝트 생성 실패:', error);
      toast.error(edit ? '프로젝트 수정에 실패했습니다.' : '프로젝트 생성에 실패했습니다.');
    }
  };
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName={edit ? "프로젝트 수정" : "프로젝트 등록"} />
